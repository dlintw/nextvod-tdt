...
/*	cresize( eSize(height(), width()) );
	...
	if ( eSystemInfo::getInstance()->getHwType() != eSystemInfo::DGS_R91 ){
		...
		colorformat->resize(eSize(120, 35));
                ...
                eDebug("[%s] v_colorformat = %d",__func__,v_colorformat);  
		...
		colorformat->setCurrent(entrys[v_colorformat]);
...
//                if (v_videoformat == 0 && v_videochange != 1) 
                eDebug("[%s] colorformat = %d", __func__, colorformat);
		eAVSwitch::getInstance()->reloadSettings();
...
public:
	eWizardTVSystemInit()
	{
		...
		FILE *f=fopen("/etc/videomode", "r");
		if (f){
			fscanf(f, "%d", &videoformat);
			eConfig::getInstance()->setKey("/elitedvb/video/videoformat", videoformat);
		}  
		fclose(f);
		system("/etc/init.d/videomode stop");
        }  
