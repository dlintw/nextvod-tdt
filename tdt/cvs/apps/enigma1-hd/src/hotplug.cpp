		...
		case POLLPRI:
		{
			...
			if ( !paramsleft )
			{
				for (std::map<eString,eString>::iterator it( params.begin() ); it != params.end(); it++ ){
					...
					if (strncmp(it->second.c_str(),"add", 3 ) == 0)
					       system("/usr/local/sbin/mount.sh");
					if (strncmp(it->second.c_str(),"remove", 6 ) == 0)
					       system("/usr/local/sbin/umount.sh");       
				}       
			}
#endif
			break;
