...
#define TMP_IMAGE "/tmp/flash_img_kernel_root"
...
void eUpgrade::flashImage(int checkmd5)
{
	...
	} else
	{
		...
		} else
		{
			...
			system("cp /sbin/reboot /tmp/reboot");				
			system("cp /bin/cubefpctl /tmp/cubefpctl");				
			int mtdsize;
			if (res == eMessageBox::btYes)
			{
				...
				if ( flashext )
				{
					...
					::close(fd1);
					__u8 data[(var->xres)*(var->yres)];
					gPixmap pixmap;
					pixmap.x=var->xres;
					pixmap.y=var->yres;
					pixmap.bpp=8;
					pixmap.bypp=1;
					pixmap.stride=var->xres;
					pixmap.data=data;
					...
					virtualRoot.resize(eSize(var->xres, var->yres));
...
//								eDebug("saved update pic 1");
							...
							if ( fd >= 0 )
							{
								...
								write(fd, pixmap.data, (var->xres*var->yres*pixmap.bpp/8));
								::close(fd);
							}
						}
						{
