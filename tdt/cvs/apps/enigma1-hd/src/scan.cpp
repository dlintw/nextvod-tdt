...
void tsManual::init_tsManual( eWidget *LCDTitle, eWidget *LCDElement)
{
	...
	b_retune=new eButton(this);
	b_retune->setName("retune");
	b_retune->setShortcut("red");
	...
	CONNECT(b_retune->selected, tsManual::retune);
		...
		case stateManual:
		{
			...
			if ( oldTp.isValid() )
				...
				case eSystemInfo::feCable:
					...
					break;
				case eSystemInfo::feSatellite:
					transponder.setSatellite(12551500, 22000000, eFrontend::polVert, 4, 0, 0, 0,0,0,0);	// some astra transponder
					break;
				case eSystemInfo::feTerrestrial:
					// most is AUTO
