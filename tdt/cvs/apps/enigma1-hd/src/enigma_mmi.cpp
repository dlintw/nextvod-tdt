...
long LengthField(unsigned char *lengthfield,long maxlength,int *fieldlen)
{
	...
	if(lengthfield[0] & 0x80)
	{
		*fieldlen = ByteCount + 1;
		...
		{
			tmp[indexVar]=lengthfield[indexField--];
		}
	}
	else
		*fieldlen = 1;
...
void enigmaMMI::gotMMIData( const char* data, int len )
{
	...
	// be deleted.
	if ((len >= 1) && ((unsigned char)data[0] != 0x9f))
		clear = 0;
	if ((len >= 2) && ((unsigned char)data[1] != 0x88))
		clear = 0;
	if (clear)
		buffer.clear();

	buffer.write( (unsigned char *)data, len );
	while ( buffer.size() >= (3 + MIN_LENGTH_BYTES) )
	{
		...
		if ( buffer.size() >= messageLength )
		{
			...
			for (int i=0; i < messageLength; ++i)
				eDebugNoNewLine("%x ", (unsigned char)dest[i]);
			eDebug("");
#endif
...
bool enigmaMMI::handleMMIMessage(const char *data)
{
	...
	while ((unsigned char)data[rp] != 0x9F || (unsigned char)data[rp+1] != 0x88 )
		rp++;

	if( !memcmp((unsigned char *)data+rp,TAG_MMI_CLOSE,TAG_LENGTH) )
	{
		rp += 3;
		int LengthBytes;
...
//			eDebug("start closeTimer %d", delay );
	...
	else if( !memcmp((unsigned char *)data+rp,TAG_MMI_ENQ,TAG_LENGTH) )
	{
		eDebug("mmi_enq_last");
		closeTimer.stop();
		...
		int blind=(unsigned char)data[rp++] & 1;  //blind_answer
	...
	else if( memcmp((unsigned char *)data+rp,TAG_MMI_MENU_LAST,TAG_LENGTH)==0 ||
		 memcmp((unsigned char *)data+rp,TAG_MMI_LIST_LAST,TAG_LENGTH)==0)
	{
			...
			if(memcmp(data+rp,TAG_MMI_TEXT_LAST,TAG_LENGTH)==0)
			{
				...
				switch (currElement)
				{
					...
					case 3:
						...
						break;
					default:
						eDebug("new entry text %s %d", text, size);
// FIXME shutdown enigmy pri zobrazeni menu CI
						entrys.push_back( std::pair<eString, int>( convertDVBUTF8(text,size), currElement-3 ) );
						entrys.push_back( std::pair<eString, int>( convertDVBUTF8(text,size), 0) );
						break;
				}
				rp += size;
			}
	...
	else if(!memcmp((unsigned char *)data+rp,TAG_MMI_MENU_MORE,TAG_LENGTH))
		eDebug("mmi_menu_more.. unhandled yet");
	else if(!memcmp((unsigned char *)data+rp,TAG_MMI_LIST_MORE,TAG_LENGTH))
		eDebug("mmi_list_more.. unhandled yet");
	else if(!memcmp((unsigned char *)data+rp,TAG_MMI_DISPLAY_CONTROL,TAG_LENGTH))
		eDebug("DISPLAY CONTROL .. still answered in dvbci");
	else
	{
		eDebug("unknown MMI_TAG:%02x%02x%02x",(unsigned char *)data[rp],(unsigned char *)data[rp+1],(unsigned char *)data[rp+2]);
		eDebug("CLOSE");
		closeMMI();
	}
	return true;
