...
static void getDefaultGateway(__u32 &ip)
{
	...
	while(fgets(buf,sizeof(buf),file))
	{
		if (sscanf(buf,"%8s %x %2x%2x%2x%2x", iface, &dest, &ipr[0],&ipr[1],&ipr[2],&ipr[3]) == 6)
		{
			if (!dest)
			{       gw=ipr[3]<<24|ipr[2]<<16|ipr[1]<<8|ipr[0];
                                ip = gw;
//                              eDebug("dhcp 0x%x (%x %x %x %x", ip,ipr[3],ipr[2],ipr[1],ipr[0]); 
				break;
			}
		}
	}
