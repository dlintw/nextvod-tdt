...
struct enigmaMainActions
{
	...
	eAction showMainMenu, standby_press, standby_nomenu_press, standby_repeat, standby_release,
		...
		showSubservices, showAudioOrPause, showAudio, pluginVTXT, pluginExt, pluginAll, showEPGList, showEPG,
	...
	enigmaMainActions():
		...
		pluginVTXT(map, "pluginVTXT", _("show Videotext"), eAction::prioDialog),
 		pluginAll(map, "pluginAll", _("show allinone Plugins"), eAction::prioDialog),
...
//	ASSIGN(Progress, eProgress, "progress_bar");
	...
	ClockFull=new eLabel(this);
	ASSIGN(ClockFull, eLabel, "full_time");

	ASSIGN(date, eLabel, "date");
	ASSIGN(Picon, eLabel, "picon");
...
void eZapMain::showFullTime()
{
	eString sf;
	time_t c=time(0)+eDVB::getInstance()->time_difference;
	tm *t=localtime(&c);
	sf.sprintf("%02d:%02d:%02d", t->tm_hour, t->tm_min, t->tm_sec);
	ClockFull->setText(sf);
}
...
/* SNR,AGC DISPLAY end */
        if(!fullTimer)
	{
		fullTimer=new eTimer(eApp);
		fullTimer->start(1000,false);
		CONNECT(fullTimer->timeout,eZapMain::showFullTime);
        }
}
...
//			eDebug("filename = descr = %s", descr.c_str() );
  ...
  // no faked service types in recordings.epl
		...
		eMessageBox msg(servicename + " - " + descr,_("record"),
		eMessageBox::btOK|eMessageBox::iconInfo, eMessageBox::btOK, 5 );
		msg.show();
		msg.exec();
		msg.hide();
		return 0;
...
void eZapMain::runPluginAllinone()
{
 	...
 	if ( checkPin( setuppin, _("setup") ) )
	{
		...
		do
		{
		    eZapPlugins plugins(eZapPlugins::IpboxPlugin);
 		    i=plugins.execPluginByName("allinone.cfg");
 		    if (!doHideInfobar())
 		    show();
		}
		while(i==-1);
	...
	if (state & stateRecording)
	{
		...
		if(lcdmain.lcdMain->Clock->isVisible()){
			lcdmain.lcdMain->Clock->hide();
          		system("echo 'recording...' > /dev/dbox/fp0");
          	}
		else {
		        lcdmain.lcdMain->Clock->show();
          		system("echo 'RECORDING...' > /dev/dbox/fp0");
...
int eZapMain::eventHandler(const eWidgetEvent &event)
{
	...
	case eWidgetEvent::evtAction:
	{
		...
		else if (event.action == &i_enigmaMainActions->showAudioOrPause)
		{
			...
			runPluginExt();
		else if (event.action == &i_enigmaMainActions->pluginAll)
			runPluginAllinone();
...
#ifndef DISABLE_LCD
                else if ( eZapStandby::getInstance() ) //  in standby
                {
                        eString time = "echo '"+s+"' > /dev/dbox/fp0";
			system(time.c_str());
		}
		else
		{
