...
void eZap::init_eZap(int argc, char **argv)
{
	...
	desktop_fb->resize(eSize(var->xres, var->yres));
	...
	if ( bootcount == 1 ){
      ...
      while(1){
		...
		eMessageBox msg(videomod,
				_("First start of enigma"),
		eMessageBox::iconQuestion|eMessageBox::btYes|eMessageBox::btNo, eMessageBox::btNo, 15 );
		msg.show();
		int res=msg.exec();
		msg.hide();
...
//		eConfig::getInstance()->setKey("/ezap/rc/sselect_style", "sselect_classic" );
	...
	*/
	...
	if ( bootcount == 1 )
	{
		eMessageBox msg (_("Welcome to ABCOM enigma HD.\n\n"
				"Yes to restore abcom setting .\n No to user setup your box ."),
				_("First start of enigma"),
