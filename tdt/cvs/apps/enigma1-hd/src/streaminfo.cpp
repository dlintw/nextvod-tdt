...
eString getVidFormat()
{
	...
	if (Decoder::current.vpid!=-1){
		fxres=fopen("/proc/stb/vmpeg/0/xres", "rt");
		if (fxres)
		{
			fscanf(fxres, "%lx", &xres);
			fclose(fxres);
		}
	
		fyres=fopen("/proc/stb/vmpeg/0/yres", "rt");
		if (fyres)
		{
			fscanf(fyres, "%lx", &yres);
			fclose(fyres);
		}
             

		faspect=fopen("/proc/stb/vmpeg/0/aspect", "rt");
		if (faspect)
		{
			fscanf(faspect, "%lx", &aspect);
			fclose(faspect);
		}

		fframerate=fopen("/proc/stb/vmpeg/0/framerate", "rt");
		if (fframerate)
		{
			fscanf(fframerate, "%lx", &framerate);
			fclose(fframerate);
		}
