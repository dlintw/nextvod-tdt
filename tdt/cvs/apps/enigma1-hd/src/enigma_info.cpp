...
{
	eLabel *machine, *processor, *frontend, *tuner, *harddisks, *vendor, *logo, *version, *dreamlogo, *triaxlogo, *ipboxlogo, *fpversion;
	eButton *okButton;
...
public:
	eAboutScreen()
	{
		...
		ipboxlogo=new eLabel(this);
		ipboxlogo->setName("ipboxlogo");
		...
		tuner->setText(eSystemInfo::getInstance()->getTuner());
		...
		if ( eSystemInfo::getInstance()->hasHDD() )
		{
			for (int c='a'; c<'e'; c++)
			{
				...
				if (ok)
				{
					FILE *f=fopen(eString().sprintf("/sys/block/sd%c/device/vendor", c).c_str(), "r");
					if (!f)
						continue;
					*line=0;
					fgets(line, 1024, f);
					...
					f=fopen(eString().sprintf("/sys/block/sd%c/device/model", c).c_str(), "r");
					if (!f)
						continue;
					*line=0;
					fgets(line, 1024, f);
					...
					f=fopen(eString().sprintf("/sys/block/sd%c/size", c).c_str(), "r");
					if (!f)
						continue;
					int capacity=0;
					fscanf(f, "%d", &capacity);
...
//				eString time=verid.mid(12, 4);
		...
		if ( !strcmp(eSystemInfo::getInstance()->getManufacturer(),"Triax") )
			...
			dreamlogo->show();
		else if ( !strcmp(eSystemInfo::getInstance()->getManufacturer(),"AB-COM") )
			ipboxlogo->show();
