...
void eTransponderWidget::init_eTransponderWidget(eWidget *parent, int edit, int type)
{
	...
	system=new eListBox<eListBoxEntryText>(this, l, edit);;
	...
	modulation=new eListBox<eListBoxEntryText>(this, l, edit);
	modulation->setName("modulation");
	if ( type & deliverySatellite )
	{
		modulationEntry[0]=new eListBoxEntryText(modulation, _("QPSK"), (void*)1);		
		modulationEntry[1]=new eListBoxEntryText(modulation, _("8PSK"), (void*)2);
		modulationEntry[2]=new eListBoxEntryText(modulation, _("AUTO"), (void*)0);		
	}
	CONNECT(modulation->selected, eTransponderWidget::updated1);

	l = new eLabel(this);
	l->setName( "lRol" );

	rolloff=new eListBox<eListBoxEntryText>(this, l, edit);
	rolloff->setName("rolloff");
	if ( type & deliverySatellite )
	{
		rolloffEntry[0]=new eListBoxEntryText(rolloff, _("35"), (void*)0);
		rolloffEntry[1]=new eListBoxEntryText(rolloff, _("25"), (void*)1);
		rolloffEntry[2]=new eListBoxEntryText(rolloff, _("20"), (void*)2);
		rolloffEntry[3]=new eListBoxEntryText(rolloff, _("AUTO"), (void*)3);						
	}
	CONNECT(rolloff->selected, eTransponderWidget::updated1);

	pilot=new eListBox<eListBoxEntryText>(this, l, edit);
	pilot->setName("pilot");
	if ( type & deliverySatellite )
	{
		pilotEntry[0]=new eListBoxEntryText(pilot, _("AUTO"), (void*)2);
		pilotEntry[1]=new eListBoxEntryText(pilot, _("ON"), (void*)0);
		pilotEntry[2]=new eListBoxEntryText(pilot, _("OFF"), (void*)1);

	}
	CONNECT(pilot->selected, eTransponderWidget::updated1);	
...
int eTransponderWidget::setTransponder(const eTransponder *transponder)
{
	...
	case deliverySatellite:
		...
		system->setCurrent(systemEntry[transponder->satellite.system&1]);;		
		modulation->setCurrent(modulationEntry[transponder->satellite.modulation&1]);
		rolloff->setCurrent(rolloffEntry[transponder->satellite.rolloff&1]);
		pilot->setCurrent(pilotEntry[transponder->satellite.pilot&1]);
	...
	case deliverySatellite:
		...
		transponder->setSatellite(
			...
			sat->getCurrent() && sat->getCurrent()->getKey() ?
				...
				: 0,
			inversion->isChecked(),
			(int)system->getCurrent()->getKey(),  
			(int)modulation->getCurrent()->getKey(),  
                        (int)rolloff->getCurrent()->getKey(),  
                        (int)pilot->getCurrent()->getKey()); 
		return 0;
	case deliveryTerrestrial:
