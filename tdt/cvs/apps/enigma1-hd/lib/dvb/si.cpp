...
Descriptor *Descriptor::create(descr_gen_t *descr, int tsidonid)
{
	...
	case 9:
		return new CADescriptor((ci_descr_t*)descr);
	case DESCR_NW_NAME:
...
CADescriptor::CADescriptor(ci_descr_t *descr)
	:Descriptor((descr_gen_t*)descr)
{
	init_CADescriptor(descr);
}
void CADescriptor::init_CADescriptor(ci_descr_t *descr)
{
	if ( len > 0 )
	{
		data=new __u8[len];
...
void SatelliteDeliverySystemDescriptor::init_SatelliteDeliverySystemDescriptor(descr_satellite_delivery_system_struct *descr)
{
	...
	FEC_inner=descr->fec_inner;
        system=descr->system;
        rolloff=descr->rolloff;
//      if(system ==1) 
//	eDebug("[%s] %d/%d/%d/%d/none/%d/%d/%d"
//	,__FUNCTION__, frequency, symbol_rate, polarisation, FEC_inner, modulation, system,rolloff);
