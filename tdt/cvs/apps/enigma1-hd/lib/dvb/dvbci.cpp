...
//	eDebug("got mmi_answer");
	...
	for(;i<MAX_SESSIONS;++i)  // search mmi session .. hope it exist only one :)
		if(sessions[i].state && sessions[i].service_class==0x41004000)
		{
			buffer[3]=i;  // session
...
//		eDebug("ca_pmt_list_management = %d", capmt[9]);
		...
		for(int i=1;i<MAX_SESSIONS;++i)
			if(sessions[i].state 
			&& (sessions[i].service_class==0x41000300 || sessions[i].service_class==0x00410300))
		{
...
//			eDebug("queue empty.. try to send data");
			...
			if ( !sendData(tc_id,buffer,len+5) )
			{
...
*/
				...
				{
					buffer[3]=session;
					if ( sessions[session].service_class==0x41000300 ||
						sessions[session].service_class==0x00410300 )
						sendTPDU(0xA0,8,sessions[session].tc_id,buffer);
					else
...
void eDVBCI::handle_session(unsigned char *data,int len)
{
	...
	else if(data[4]==0x9f && data[5]==0x80 && data[6]==0x31)
	{
		...
		{
			if ( sessions[data[3]].service_class==0x41000300  ||
				 sessions[data[3]].service_class==0x00410300)
				ca_manager(data[3]);
			else
...
int eDVBCI::service_available(unsigned long service_class)
{
	switch(service_class)
	{
		...
		case 0x41000300: //if it is so...perhaps its better you switch the xa to 8bit mode *g*
		case 0x00410300:
	...
	switch(data[0])
	{
		...
		case 0x91:
			if(service_available(*(unsigned long*)(data+2)))
			{
				...
				if(sessions[i].service_class==0x41000100)
					help_manager(i);
				if(sessions[i].service_class==0x41000200)
					app_manager(i);
				if(sessions[i].service_class==0x41000300)
					ca_manager(i);
				if(sessions[i].service_class==0x00410300)
					ca_manager(i);
			}
			else
			{
		...
		case 0x95:		//T_close_session
			{
				...
				if( sessions[data[3]].state != STATE_FREE )
				{
					...
					if(sessions[data[3]].service_class == 0x41004000 )
					{
...
void eDVBCI::dataAvailable(int what)
{
	...
	if ( what != 1000 )
	{
		if (::ioctl(fd,CA_GET_SLOT_INFO,&info)<0)
			eDebug("[DVBCI] CA_GET_SLOT_INFOS failed (%m)");
		...
		if(present == 0)
		{
			...
			int delka=::read(fd,&buffer,0);
//			eDebug("READ 1:%d",delka);
			ci_state=0;
			clearCAIDs();
...
//	eDebug("READ 3:%d",size);
	...
	if(size>0)
	{
		...
		if (::ioctl(fd,CA_GET_SLOT_INFO, &info)<0)
			eDebug("CA_GET_SLOT_INFO failed (%m)");
...
void eDVBCI::poll()
{
	...
	if (::ioctl(fd,CA_GET_SLOT_INFO, &info)<0)
		eDebug("CA_GET_SLOT_INFO failed (%m)");
        present=info.flags;         
//      eDebug("[%s] num = %d type = %d flag = %d",__FUNCTION__, 
