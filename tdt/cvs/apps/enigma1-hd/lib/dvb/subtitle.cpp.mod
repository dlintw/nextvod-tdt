...
//					exit(0);
			...
			if ( !fbClass::getInstance()->islocked() )
				...
				{
//					eDebug("\n%d %d %d %d %d %d\n", 
//					y,reg->region_height, sub->screen_width, region->region_vertical_address,region->region_horizontal_address, reg->region_width );
...
*/
//			memcpy(sub->screen_buffer + 720 *(y+474)+0, reg->region_buffer+720*y,720);
			memcpy(sub->screen_buffer + 720 *(y+474)+0, reg->region_buffer+720*y,720);
				}
		} 
		else
