...
//	eDebug("Decoder::Flush()");
	parms.vpid = parms.apid = parms.tpid = parms.pcrpid = parms.pmtpid = parms.video_type = -1;
	if (!keepaudiotype)
		parms.audio_type=0;
...
int Decoder::Set()
{
	...
	int changed=0;
        ...
        eDebug("apid = %x : %x", parms.apid , current.apid);
        eDebug("tpid = %x : %x", parms.tpid , current.tpid);
        eDebug("pcrpid = %x : %x", parms.pcrpid , current.pcrpid);                
        eDebug("vtype = %x : %x", parms.video_type , current.video_type);        
        eDebug("pmtpid = %x : %x", parms.pmtpid , current.pmtpid);
        eDebug("descriptor = %x : %x", parms.descriptor_length , current.descriptor_length);
        eDebug("atype = %x : %x\n", parms.audio_type , current.audio_type);
#endif
	...
	if (parms.pcrpid != current.pcrpid)
		changed |= 8;
	if (parms.video_type != current.video_type )
		changed |= 0x10;
...
//				eDebug("fd.demux_pcr closed");
		...
		if ( changed & 0x1b && parms.video_type != current.video_type )
		{
//			eDebug("[%s] change video type = %d",__FUNCTION__, parms.video_type); 
            		if (parms.video_type == 27){
            		    if(::ioctl(fd.video, VIDEO_SET_ENCODING, VIDEO_ENCODING_H264)< 0)
                		eDebug("failed (%m)");
				eDebug("VIDEO TYPE = H264");
            		} else if (parms.video_type == 2){ 
                	    if(::ioctl(fd.video, VIDEO_SET_ENCODING, VIDEO_ENCODING_MPEG2)< 0)
                    		eDebug("failed (%m)");
	                	eDebug("VIDEO TYPE = MPEG2");
            		} else {
            	 	    if(::ioctl(fd.video, VIDEO_SET_ENCODING, VIDEO_ENCODING_AUTO)< 0)
                    		eDebug("failed (%m)");
	                	eDebug("VIDEO TYPE = AUTO");
	                } 
	        }
	}
	...
	if (changed & 0x100 && parms.audio_type != current.audio_type )
	{
		...
		case DECODE_AUDIO_DTS:
			bypass=2;
			break;
		case DECODE_AUDIO_MP3:
			bypass=4;
			break;
		}
		eDebugNoNewLine("AUDIO_SET_ENCODING to %d - ", bypass);
		int wasclosed = 0;
		if ( fd.audio == -1 )  // open audio dev... if not open..
		{
			fd.audio=open(AUDIO_DEV, O_RDWR);
...
/*			else
		...
		if ( bypass == 0){
                        eDebug("AUDIO_TYPE = AC3"); 
			if (::ioctl( fd.audio , AUDIO_SET_ENCODING, AUDIO_ENCODING_AC3 ) < 0)
				eDebug("failed (%m)");
			else
				eDebug("ok");
		}	
		if ( bypass == 1){
		        eDebug("AUDIO_TYPE = MPEG"); 
			if (::ioctl( fd.audio , AUDIO_SET_ENCODING, AUDIO_ENCODING_MPEG2 ) < 0)
				eDebug("failed (%m)");
			else
				eDebug("ok");
		}	
		if ( bypass == 4){
		        eDebug("AUDIO_TYPE = MP3"); 
			if (::ioctl( fd.audio , AUDIO_SET_ENCODING, AUDIO_ENCODING_MP3 ) < 0)
				eDebug("failed (%m)");
			else
				eDebug("ok");

			if (ioctl(fd.audio, AUDIO_SELECT_SOURCE, AUDIO_SOURCE_MEMORY) <0)
				eDebug("failed (%m)");
			else
				eDebug("ok");
	
			if (ioctl(fd.audio, AUDIO_SET_STREAMTYPE, STREAM_TYPE_ES) <0)
				eDebug("failed (%m)");
			else
				eDebug("ok");
			...
			if (ioctl(fd.audio, AUDIO_SET_SPDIF_SOURCE, AUDIO_SPDIF_SOURCE_PP) <0)
				eDebug("failed (%m)");
			else
				eDebug("ok");
		}	
...
//			eDebug("fd.demux_audio closed");
		...
		if ( (changed & 0x0F) != 2 )  //  only apid changed
		{
			...
			{
				eDebugNoNewLine("enabling av sync mode 1 - ");
				if (::ioctl(fd.audio, AUDIO_SET_AV_SYNC, 1)<0)
					eDebug("failed (%m)");
				else
					eDebug("ok");
		...
		if ( parms.apid != -1 )
		{
			...
			eDebugNoNewLine("enabling av sync mode 2 - ");
			if (::ioctl(fd.audio, AUDIO_SET_AV_SYNC, 1)<0)
				eDebug("failed (%m)");
			else
				eDebug("ok");
...
int Decoder::displayIFrame(const char *frame, int len)
{
	...
	for ( int i=0; i < 2; i++ )
		write(fd.video, frame, len);

	unsigned char buf[128];
	memset(&buf, 0, 128);
	write(fd.video, &buf, 128);
	return 0;
}
...
void Decoder::clearScreen()
{
	int wasOpen = fd.video != -1;
	if ( !wasOpen )
		fd.video=::open("/dev/dvb/adapter0/video0", O_RDWR);
	if ( fd.video > -1 )
	{
		if ( ::ioctl(fd.video, VIDEO_STOP) < 0 )
			eDebug("VIDEO_STOP failed (%m)");

		if ( ::ioctl(fd.video, VIDEO_SET_BLANK) < 0 )
