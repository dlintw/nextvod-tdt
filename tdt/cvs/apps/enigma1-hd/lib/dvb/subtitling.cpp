...
{
        unsigned char pesHdr[] = "\x00\x00\x01\xbd";
	while (1)
	{
...
void eSubtitleWidget::startttx(int page)
{
	...
	unsigned int vformat = 0;
	int left=20, top=20, right=699, bottom=555;
        int o_left=0, o_top=0, o_right = 0, o_bottom = 0; 
	eConfig::getInstance()->getKey("/elitedvb/video/videoformat", vformat);
//      eDebug("vformat = %d", vformat);
	switch (vformat)
	{
		case 2: 
		case 3:
		    o_left=30, o_top=10, o_right = 500, o_bottom = 144; 
		break;
		case 4: 
		case 5: 
		    o_left=100, o_top=40, o_right = 1200, o_bottom = 504; 
		break;
	}
	...
	renderinfo.sx = left+o_left;
	renderinfo.sy = top+o_top;
	renderinfo.ex = right+o_right;
	renderinfo.ey = bottom+o_bottom;
...
//	eDebug("[%s] pid =%2X visible = %d",__FUNCTION__, pid, isvisible);	
	pos = 0;
}
