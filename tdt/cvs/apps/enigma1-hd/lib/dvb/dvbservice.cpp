				...
				for (int i=0; i<len; i+=descriptor[i+1]+2)
				{
					...
					switch (tag)
					{
						...
						case eServiceDVB::cPCRPID:
							...
							break;
						case eServiceDVB::cVideoType:
							sp->dvb->set(eServiceDVB::cVideoType, (descriptor[i+2]<<8)|(descriptor[i+3]));
							break;
...
//VIDEO TYPE					
                                tmp = sp->dvb->get(eServiceDVB::cVideoType);
                                eDebug("[%s] eventServiceTuneOK: cVideoType = %d\n", __FUNCTION__, tmp); 
				if ( tmp != -1 )
				     Decoder::parms.video_type=tmp;
// start yet...
				Decoder::Set();
...
// workaround for zap in background before recordings
	...
	case eDVBServiceEvent::eventServiceFailed:
		eDebug("[TUNE] service failed %d", service_state);
		/*emit*/ dvb.switchedService(service, -service_state);
		dvb.setState(eDVBServiceState(eDVBServiceState::stateIdle));
		break;
	}
...
void eDVBServiceController::scanPMT( PMT *pmt )
{
	...
	int audiopid=-1, videopid=-1, ac3pid=-1, tpid=-1, vtype=-1;
	...
	if (sp)
	{
		if (sp->dvb)
		{
			...
			vtype=sp->dvb->get(eServiceDVB::cVideoType);		
		}
	}
	...
	if ( video )
	{
		if (( video->elementary_PID != videopid ) || ( vtype == -1 ) || (vtype == 27))
			setPID(video);
		else 
            		Decoder::parms.video_type = vtype;
	}
...
// Linkage service handling..
	...
	if ( !newservice )
	{
		...
		disableFrontendTimer.start(1000, true);
	}
/////////////////////////////////
...
void eDVBServiceController::setPID(const PMTEntry *entry)
{
	if (entry)
	{
		...
		{
			if ( entry->stream_type != 2 )
                		Decoder::parms.video_type=entry->stream_type; 		
			Decoder::current.video_type=0;

			Decoder::parms.vpid=entry->elementary_PID;
			if (sp && sp->dvb)
			{
				sp->dvb->set(eServiceDVB::cVideoType, entry->stream_type);
