	...
	void operator()(eTransponder &t)
	{
		...
		if (t.satellite.valid)
				fprintf(f, "\ts %d:%d:%d:%d:%d:%d:%d:%d:%d:%d",
				 t.satellite.frequency,
				 t.satellite.symbol_rate,
				 t.satellite.polarisation,
				 t.satellite.fec,
				 t.satellite.orbital_position,
				 t.satellite.inversion,
				 t.satellite.system,
				 t.satellite.modulation,
				 t.satellite.rolloff,
				 t.satellite.pilot);
...
void eDVBSettings::loadServices()
{
	...
	while (!feof(f))
	{
		...
		while (!feof(f))
		{
			...
			if (line[1]=='s')
			{
				int frequency, symbol_rate, polarisation, fec, sat, inversion=INVERSION_OFF, system=0, modulation=0, rolloff=0, pilot=2;
				if (version == 2)
					sscanf(line+2, "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d", &frequency, &symbol_rate, &polarisation, &fec, &sat, &inversion , &onlyFree);
				else
					sscanf(line+2, "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d", &frequency, &symbol_rate, &polarisation, &fec, &sat, &inversion , &system, &modulation, &rolloff, &pilot, &onlyFree);	
				t.setSatellite(frequency, symbol_rate, polarisation, fec, sat, inversion, system, modulation, rolloff, pilot);
			}
			if (line[1]=='c')
			{
		...
		while (!feof(f))
		{
			...
			bouquet->add(
				eServiceReferenceDVB(
					...
					service_type));
//                                      if (service_type == 25)
//					        eDebug("[%s] service_id = %x:%d  service type = %d", __FUNCTION__, service_id, service_id, service_type);					
		}
	}
...
int eDVBSettings::importSatcoDX(eString line)
{
	...
	t.setSatellite(atoi(frequency.c_str()), atoi(symbolrate.c_str())*1000, atoi(polarization.c_str())^1, myfec, my_orbital_position, 0, 0, 0, 0, 0);
