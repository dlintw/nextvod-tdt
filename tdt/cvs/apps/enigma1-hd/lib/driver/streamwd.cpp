...
void eStreamWatchdog::reloadSettings(int override_aspect)
{
	...
	FILE *faspect=fopen("/proc/stb/vmpeg/0/aspect", "rt");
	if (faspect)
	{
		int aspect=0;
	...
	eConfig::getInstance()->getKey("/elitedvb/video/pin8", pin8);
        eDebug("[%s] doanamorph = %d isanamorph = %d pin8 = %d",
        __FUNCTION__,doanamorph, isanamorph, pin8);
	switch (pin8)
	{
		case 0:
			doanamorph=0;
	...
	FILE *ffrate=fopen("/proc/stb/vmpeg/0/framerate", "rt");	
	if (ffrate)
	{
		int frate=23976;
		fscanf(ffrate, "%lx", &frate);
		fclose(ffrate);
