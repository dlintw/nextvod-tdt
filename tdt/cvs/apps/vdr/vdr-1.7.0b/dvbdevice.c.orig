...
bool cDvbTuner::SetFrontend(void)
{
  ...
  if (frontendType & (DVBFE_DELSYS_DVBS | DVBFE_DELSYS_DVBS2)) {
     ...
     if (Frontend.delivery == DVBFE_DELSYS_DVBS)  {
        Frontend.delsys.dvbs.modulation = DVBFE_MOD_QPSK;//dvbfe_modulation(channel.Modulation());
        Frontend.delsys.dvbs.symbol_rate = channel.Srate() * 1000UL;
        Frontend.delsys.dvbs.fec = dvbfe_fec(channel.CoderateH());
        printf("DVB-Sde1: modulation=%u, symbol_rate=%u, fec=%u\n",Frontend.delsys.dvbs.modulation,Frontend.delsys.dvbs.symbol_rate,Frontend.delsys.dvbs.fec);
        } 
     else  {
        Frontend.delsys.dvbs2.modulation = dvbfe_modulation(channel.Modulation());
        printf("DVB-S2: modulation=%u, symbol_rate=%u, fec=%u\n",Frontend.delsys.dvbs2.modulation,Frontend.delsys.dvbs2.symbol_rate,Frontend.delsys.dvbs2.fec);
...
cDvbDevice::cDvbDevice(int n)
{
  ...
  printf("fd_video=%d, fd_audio=%d\n", fd_video, fd_audio);

   int card = DMX_SOURCE_FRONT0 + n;
   if (ioctl(fd_stc, DMX_SET_SOURCE, &card) < 0) {
      printf("DMX_SET_SOURCE failed");
   }
//Trick: geht zur Zeit nicht!!!
  // Common Interface:
...
{
printf("cDvbDevice::%s:%d Type=%d %s ->\n", __func__, __LINE__, Type, On?"On":"Off");
  ...
  if (Handle->pid) {
	...
	if (ioctl(Handle->handle, DMX_START) < 0) {
           LOG_ERROR;
           return false;
           }
        }
...
printf("cDvbDevice::%s:%d - TurnOnLivePIDs=%d\n", __func__, __LINE__, TurnOnLivePIDs);
...
bool cDvbDevice::HasLock(int TimeoutMs)
{
  ...
  if ((dvbTuner ? dvbTuner->Locked(TimeoutMs) : false)  && !lock) {
     ...
     if(DVBFE_DELSYS_DVBS){
	printf("DVBFE_DELSYS_DVBS\n");
        CHECK(ioctl(fd_video, VIDEO_SET_ENCODING, 2 /* mpeg2 */));
        CHECK(ioctl(fd_audio, AUDIO_SET_ENCODING, 4 /* mpeg2 */));
     }
     else if(DVBFE_DELSYS_DVBS2)
     {
	printf("DVBFE_DELSYS_DVBS\n");
        CHECK(ioctl(fd_video, VIDEO_SET_ENCODING, 8 /* h264 */));
        CHECK(ioctl(fd_audio, AUDIO_SET_ENCODING, 6 /* ac3 */));
     }
...
printf("pmExtern_THIS_SHOULD_BE_AVOIDED\n");
         close(fd_video);
         close(fd_audio);
         fd_video = fd_audio = -1;
         break;
