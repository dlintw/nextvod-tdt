    ...
    cListObject
{
  private:
    ...
    uchar * data;
  public:
    cCaDescriptor(int CaSystem, int CaPid, bool Stream, int Length,
                  const uchar * Data);
...
cCaDescriptor::cCaDescriptor(int CaSystem, int CaPid, bool Stream, int Length,
                             const uchar * Data)
{
    caSystem = CaSystem;
    stream = Stream;
...
class cCaDescriptors:public cListObject
{
  ...
  public:
    ...
    int GetCaDescriptors(const unsigned short *CaSystemIds, int BufSize,
...
{
    return Is(CaDescriptors->source, CaDescriptors->transponder,
              CaDescriptors->serviceId);
}
...
{
    cCaDescriptor *nca =
        new cCaDescriptor(d->getCaType(), d->getCaPid(), Stream,
                          d->privateData.getLength(),
                          d->privateData.getData());
    for (cCaDescriptor * ca = caDescriptors.First(); ca;
         ca = caDescriptors.Next(ca))
    {
        if (*ca == *nca)
        {
            delete nca;
...
int cCaDescriptors::GetCaDescriptors(const unsigned short *CaSystemIds,
                                 int BufSize, uchar * Data, bool & StreamFlag)
{
    ...
    if (BufSize > 0 && Data)
    {
        int length = 0;
        int IsStream = -1;
        for (cCaDescriptor * d = caDescriptors.First(); d;
             d = caDescriptors.Next(d))
        {
...
class cCaDescriptorHandler:public cList < cCaDescriptors >
{
  ...
  public:
    ...
    int GetCaDescriptors(int Source, int Transponder, int ServiceId,
                         const unsigned short *CaSystemIds, int BufSize,
                         uchar * Data, bool & StreamFlag);
};
...
int cCaDescriptorHandler::GetCaDescriptors(int Source, int Transponder,
                                       int ServiceId,
                                       const unsigned short *CaSystemIds,
                                       int BufSize, uchar * Data,
                                       bool & StreamFlag)
{
    cMutexLock MutexLock(&mutex);
    StreamFlag = false;
    for (cCaDescriptors * ca = First(); ca; ca = Next(ca))
    {
        if (ca->Is(Source, Transponder, ServiceId))
            return ca->GetCaDescriptors(CaSystemIds, BufSize, Data,
                                        StreamFlag);
    }
    return 0;
}
...
int GetCaDescriptors(int Source, int Transponder, int ServiceId,
                 const unsigned short *CaSystemIds, int BufSize, uchar * Data,
                 bool & StreamFlag)
{
    return CaDescriptorHandler.GetCaDescriptors(Source, Transponder,
                                                ServiceId, CaSystemIds,
                                                BufSize, Data, StreamFlag);
}
...
void
PatFilter::SetSdtFilter(SdtFilter * SdtFilter)
{
    sdtFilter = SdtFilter;
    SetStatus(true);
}
...
void
PatFilter::Process(u_short Pid, u_char Tid, const u_char * Data, int Length)
{
    ...
    if (Pid == 0x00)
    {
        if (Tid == 0x00)
        {
            ...
            {
                if (pmtPid[i]
                    && (time(NULL) - lastPmtScan[i]) > FILTERTIMEOUT)
                {
            ...
            for (SI::Loop::Iterator it;
                 pat.associationLoop.getNext(assoc, it);)
            {
                ...
                if (!sidfound && !assoc.isNITPid()
                    && SidinSdt(assoc.getServiceId()))
                {
    ...
    else if (Tid == SI::TableIdPMT && Source() && Transponder())
    {
        ...
        cChannel *Channel =
            Channels.GetByServiceID(Source(), Transponder(),
                                    pmt.getServiceId());
        if (Channel && Index != -1)
        {
            SI::CaDescriptor * d;
            cCaDescriptors *CaDescriptors =
                new cCaDescriptors(Channel->Source(), Channel->Transponder(),
                                   Channel->Sid());
            // Scan the common loop:
            for (SI::Loop::Iterator it;
                 (d =
                  (SI::CaDescriptor *) pmt.commonDescriptors.getNext(it,
                                                                     SI::
                                                                     CaDescriptorTag));)
            {
	    ...
	    char SLangs[MAXSPIDS + 1][MAXLANGCODE2] = { "" };
		    ...
		    break;
                                ...
                                case SI::ISO639LanguageDescriptorTag:
                                    {
                                        SI::ISO639LanguageDescriptor * ld =
                                            (SI::
                                             ISO639LanguageDescriptor *) d;
                                        if (*ld->languageCode != '-')
                                        {       // some use "---" to indicate "none"
                                            strn0cpy(ALangs[NumApids],
                ...
                for (SI::Loop::Iterator it;
                     (d =
                      (SI::CaDescriptor *) stream.streamDescriptors.
                      getNext(it, SI::CaDescriptorTag));)
                {
...
#if APIVERSNUM >= 10715
            Channel->SetPids(Vpid, Ppid, Vtype, Apids, Atypes, ALangs,
            ...
            Channel->SetCaDescriptors(CaDescriptorHandler.
                                      AddCaDescriptors(CaDescriptors));
...
void
PatFilter::GetFoundNum(int &current, int &total)
{
    current = numRunning;
...
void
SdtFilter::Process(u_short Pid, u_char Tid, const u_char * Data, int Length)
{
  ...
  DEBUG_printf("\nSdtFilter::Process IN %s",strDate);
    ...
    if (!sectionSyncer.
        Sync(sdt.getVersionNumber(), sdt.getSectionNumber(),
    ...
    for (SI::Loop::Iterator it; sdt.serviceLoop.getNext(SiSdtService, it);)
    {
        ...
        for (SI::Loop::Iterator it2;(d = SiSdtService.serviceDescriptors.getNext(it2));)
        {
            ...
            case SI::ServiceDescriptorTag:
                {
                    ...
                    sd->serviceName.getText(NameBufDeb, ShortNameBufDeb,
                                            sizeof(NameBufDeb),
                                            sizeof(ShortNameBufDeb));
                    DEBUG_SDT(DBGSDT
                              " \033[0;43m  Name %s --  ServiceType: %X: AddServiceType %d, Sid %i, running %i \033[0m\n",
                              NameBufDeb, sd->getServiceType(),
                              AddServiceType, SiSdtService.getServiceId(),
                              SiSdtService.getRunningStatus());
                    ...
                    case 0x86: // ?? Astra 28.2 MPEG-4 HD

                        {
                           ...
                           // esyslog("%s sd->getServiceType()=%d  AddServiceType=%d",__FUNCTION__, sd->getServiceType(), AddServiceType);
                            if (!(sd->getServiceType() == 0x11
                                || sd->getServiceType() == 0x19) &&
                                 AddServiceType == ST_HDTV_ONLY) // (! HD TV && HDOnly) break
                            {
                              DEBUG_SDT(DBGSDT
                                          "  \033[0;48m +++++++++++++++  NO Found HD CHANNEL: Skip %s +++++++++++++++ \033[0m \n",
                                          NameBufDeb);
                             break;
                            }
                            ...
                            // Add only radio
                            if ((sd->getServiceType() == 1
                                 || sd->getServiceType() == 0x11 || sd->getServiceType() == 0x19
                                 || sd->getServiceType() == 0xC3)
                                 
                                 && AddServiceType == ST_RADIO_ONLY)// (TV && radioOnly) break
                            {
                                DEBUG_SDT(DBGSDT
                                          " Add nur Radio  aber nur TV Sender gefunden  SID skip %d \n",
                                          sd->getServiceType());
                                break;
                            }
                            // Add only tv
                            if (sd->getServiceType() == 2
                                && (AddServiceType == ST_TV_ONLY || AddServiceType == ST_HDTV_ONLY ))// RadioSender && (TVonly || HDTvonly) break
                            {
                                DEBUG_SDT(DBGSDT
                                          " Add nur TV  aber nur RadioSender gefunden  SID skip %d \n",
                                          sd->getServiceType());
                                break;
                            }
                            char NameBuf[1024];
                            ...
                            {
                                DEBUG_SDT(DBGSDT
                                          " \033[0;42m---------------------- Channelscan Add Chanel pn %s ps %s pp %s --------------\033[0m\n",
                                          pn, ps, pp);
                                channel->SetId(sdt.getOriginalNetworkId(),
                            ...
                            {
                                channel =
                                    Channels.NewChannel(Channel(), pn, ps, pp, sdt.getOriginalNetworkId(),
                                                        sdt.getTransportStreamId(),
                                                        SiSdtService.getServiceId());
#if defined(REELVDR)
                                channel->SetServiceType(sd->getServiceType());
                                ...
                                {
                                    DEBUG_SDT(DBGSDT
                                              "-------- found ServiceID for PremiereDirekt!  %s - %s - %s --------- \n",
                                              pn, ps, pp);
                                    //eitFilter->Trigger();
                                }
                            }
            ...
            case SI::NVODReferenceDescriptorTag:
                {
                    SI::NVODReferenceDescriptor * nrd =
                        (SI::NVODReferenceDescriptor *) d;
                    SI::NVODReferenceDescriptor::Service Service;
                    for (SI::Loop::Iterator it;
                         nrd->serviceLoop.getNext(Service, it);)
                    {
                        cChannel *link =
                            Channels.
                            GetByChannelID(tChannelID
                                           (Source(),
                        ...
                        if (!link)
                        {
                            ...
                            sid[numSid++] = SiSdtService.getServiceId();
                            link =
                                Channels.NewChannel(Channel(), "NVOD", "", "",
                                                    Service.
...
void
NitFilter::Process(u_short Pid, u_char Tid, const u_char * Data, int Length)
{
        ...
        for (int i = 0; i < numNits; i++)
        {
            ...
            if (nits[i].networkId == nit.getNetworkId())
            {
                if (nit.getSectionNumber() == 0)
                {
                    ...
                    {
                        DEBUG_NIT(DBGNIT
                                  " --  nits[%d] has Transponder ? %s \n", j,
                                  nits[j].hasTransponder ? "YES" : "NO");
                        if (nits[j].hasTransponder)
                        {
                            networkId = nits[j].networkId;
                            DEBUG_NIT(DBGNIT
                                      " take  NIT with network ID %d\n",
                                      networkId);
                    ...
                    {
                        DEBUG_NIT(DBGNIT
                                  "none of the NITs contains transponder %d\n",
                                  Transponder());
                        return;
                    }
                }
        ...
        if (!networkId && ThisNIT < 0 && numNits < MAXNITS)
        {
            if (nit.getSectionNumber() == 0)
            {
                *nits[numNits].name = 0;
                ...
                for (SI::Loop::Iterator it;
                     (d = nit.commonDescriptors.getNext(it));)
                {
                    switch (d->getDescriptorTag())
                    {
                    case SI::NetworkNameDescriptorTag:
                        {
                            SI::NetworkNameDescriptor * nnd =
                                (SI::NetworkNameDescriptor *) d;
    ...
    else if (!sectionSyncer.
             Sync(nit.getVersionNumber(), nit.getSectionNumber(),
                  nit.getLastSectionNumber()))
        return;
    ...
    for (SI::Loop::Iterator it; nit.transportStreamLoop.getNext(ts, it);)
    {
        ...
        SI::FrequencyListDescriptor * fld =
            (SI::FrequencyListDescriptor *) ts.transportStreamDescriptors.
            getNext(it2, SI::FrequencyListDescriptorTag);
...
//                        int Modulation = QPSK;
                        ...
                        {       // HD
                            if (Frequency == 11914)
                                ...
                                Modulation =
                                    ModulationsS2[sd->getModulationType()];

                            if (Frequency == 11914)
                                printf
                                    ("11914 SI f %d sr:%d mod:SI %d->%d code:SI%d->%d ro:xx->%d \n",
                                     Frequency, SymbolRate,
                                     sd->getModulationType(), Modulation,
                                     sd->getFecInner(), CodeRate, RollOff);
                        }
                        ...
                        {
                            esyslog
                                ("FatalError new cSatTransponder %d failed",
                                 Frequency);
                        }
                        else
                        {
    ...
    {
        printf
            ("DEBUG [channescan ]: filter.c  End of ProcessLoop newMap size: %d  \n",
