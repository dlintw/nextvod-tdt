...
int mpeg_hdr_debug = 1;
#define mpeg_hdr_printf(x...) do { if (mpeg_hdr_debug)printf(x); } while (0)
...
float mpeg12_aspect_info(mp_mpeg_header_t *picture)
{
    ...
    switch(picture->aspect_ratio_information) {
      ...
      case 9: // Movie Type ??? / 640x480
        ...
        break;
      default:
        mpeg_hdr_printf("Detected unknown aspect_ratio_information in mpeg sequence header.\n"
               "Please report the aspect value (%i) along with the movie type (VGA,PAL,NTSC,"
               "SECAM) and the movie resolution (720x576,352x240,480x480,...) to the EPlayer"
