...
int aviheader_debug = 1;
#define aviheader_printf(x...) do { if (aviheader_debug)printf(x); } while (0)

char *index_file_save, *index_file_load;
...
while(1){
  ...
  if(id==mmioFOURCC('L','I','S','T')){
    ...
    aviheader_printf("LIST %.4s  len=%u\n",(char *) &id,len);
    if(len >= 4) {
	len -= 4;
	list_end=stream_tell(demuxer->stream)+((len+1)&(~1));
    } else {
	aviheader_printf("DEMUX_AVIHDR_EmptyList\n");
	list_end = 0;
    }
    aviheader_printf("list_end=0x%X\n",(int)list_end);
  ...
  aviheader_printf("CHUNK %.4s  len=%u\n",(char *) &id,size2);
  chunksize=(size2+1)&(~1);
  switch(id){
	...
	priv->duration=(double)sh_video->video.dwLength/fps;
	printf("\n\nlen:%f fps:%f\n\n",priv->duration,fps);
      } else
    ...
    case mmioFOURCC('i', 'n', 'd', 'x'): {
      ...
      *(uint32_t *)s->dwChunkId = stream_read_dword_le(demuxer->stream);
	  ...
	  aviheader_printf("ODML (%.4s): [%d] 0x%016"PRIx64" 0x%04x %u\n", 
		  (s->dwChunkId), i,
...
//        if(demuxer->video->id==-1) demuxer->video->id=stream_id;
	...
	case mmioFOURCC('D', 'I', 'V', '1'):
	  ...
	  aviheader_printf("DEMUX_AVIHDR_RegeneratingKeyfTableForMPG4V1\n");
	  break;
	...
	case mmioFOURCC('D', 'I', 'V', '2'):
	  ...
	  aviheader_printf("DEMUX_AVIHDR_RegeneratingKeyfTableForDIVX3\n");
	  break;
	  ...
	  aviheader_printf("DEMUX_AVIHDR_RegeneratingKeyfTableForMPEG4\n");
	  break;
        }
      } else
...
printf("%s::%d <--\n", __FUNCTION__, __LINE__);
  ...
  aviheader_printf("list_end=0x%"PRIX64"  pos=0x%"PRIX64"  chunksize=0x%"PRIX64"  next=0x%"PRIX64"\n",
  ...
  if((int)chunksize<0) aviheader_printf("chunksize=%u  (id=%.4s)\n",chunksize,(char *) &id);
  printf("%s::%d\n", __FUNCTION__, __LINE__);
}
printf("%s::%d\n", __FUNCTION__, __LINE__);
...
if (priv->isodml && (index_mode==-1 || index_mode==0 || index_mode==1)) {
	...
	for (j=0; j<cx->nEntriesInUse; j++) {
		...
		priv->isodml = 0;
		priv->idx_size = 0;
		aviheader_printf("DEMUX_AVIHDR_BrokenODMLfile\n");
		goto freeout;
	    }
...
if (index_file_load) {
  ...
  if ((fp = fopen(index_file_load, "r")) == NULL) {
    aviheader_printf("DEMUX_AVIHDR_CantReadIdxFile %p %p\n", index_file_load, strerror(errno));
    goto gen_index;
  }
  fread(&magic, 6, 1, fp);
  if (strncmp(magic, "MPIDX1", 6)) {
    aviheader_printf("DEMUX_AVIHDR_NotValidMPidxFile %p\n", index_file_load);
    goto gen_index;
  }
  fread(&priv->idx_size, sizeof(priv->idx_size), 1, fp);
  priv->idx=malloc(priv->idx_size*sizeof(AVIINDEXENTRY));
  if (!priv->idx) {
    aviheader_printf("DEMUX_AVIHDR_FailedMallocForIdxFile %p\n", index_file_load);
    priv->idx_size = 0;
    goto gen_index;
  }
  ...
  for (i=0; i<priv->idx_size;i++) {
    ...
    if (feof(fp)) {
      aviheader_printf("DEMUX_AVIHDR_PrematureEOF %p\n", index_file_load);
      free(priv->idx);
      priv->idx_size = 0;
      goto gen_index;
    }
  ...
  fclose(fp);
  aviheader_printf("DEMUX_AVIHDR_IdxFileLoaded %p\n", index_file_load);
}
gen_index:
...
skip_chunk:
  ...
  if (index_file_save) {
    ...
    if ((fp=fopen(index_file_save, "w")) == NULL) {
      aviheader_printf("DEMUX_AVIHDR_Failed2WriteIdxFile %p %p\n", index_file_save, strerror(errno));
      return;
    }
    ...
    fclose(fp);
    aviheader_printf("DEMUX_AVIHDR_IdxFileSaved %p\n", index_file_save);
  }
}
printf("%s::%d\n", __FUNCTION__, __LINE__);
}
