...
int mp3_hdr_debug = 0;
#define mp3_hdr_printf(x...) do { if (mp3_hdr_debug)printf(x); } while (0)
...
int mp_get_mp3_header(unsigned char* hbuf,int* chans, int* srate, int* spf, int* mpa_layer, int* br){
    ...
    mp3_hdr_printf("head=0x%08X\n",newhead);

#if 1
    // head_check:
    if( (newhead & 0xffe00000) != 0xffe00000 ){
	mp3_hdr_printf("head_check failed\n");
	return -1;
    }
#endif
    ...
    if(layer==4){ 
      mp3_hdr_printf("not layer-1/2/3\n"); 
      return -1;
    }

    sampling_frequency = ((newhead>>10)&0x3);  // valid: 0..2
    if(sampling_frequency==3){
	mp3_hdr_printf("invalid sampling_frequency\n");
	return -1;
    }
...
//      mp3_hdr_printf("Free format not supported.\n");
//      return -1;
//    }
    ...
    mp3_hdr_printf("FRAMESIZE: %d, layer: %d, bitrate: %d, mult: %d\n", 
    	framesize, layer, tabsel_123[lsf][layer-1][bitrate_index], mult[layer-1]);
    if(!framesize){
	mp3_hdr_printf("invalid framesize/bitrate_index\n");
	return -1;
    }
