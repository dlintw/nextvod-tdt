...
int asfheader_debug = 0;
#define asfheader_printf(x...) do { if (asfheader_debug)printf(x); } while (0)
...
static char* get_ucs2str(const uint16_t* inbuf, uint16_t inlen)
{
  ...
  if (!outbuf) {
    asfheader_printf("MemAllocFailed\n");
    return NULL;
  }
  q = outbuf;
  for (i = 0; i < inlen / 2; i++) {
    uint8_t tmp;
//FIXME    PUT_UTF8(AV_RL16(&inbuf[i]), tmp, *q++ = tmp;)
  }
  return outbuf;
}
...
//  for(i=0;i<16;i++) printf(" %02X",asfhdrguid[i]);printf("\n");
  if(memcmp(asfhdrguid,asf->header.objh.guid,16)){
    asfheader_printf("ASF_check: not ASF guid!\n");
    free(asf);
    return 0; // not ASF guid
  }
  if(asf->header.cno>256){
    asfheader_printf("ASF_check: invalid subchunks_no %d\n",(int) asf->header.cno);
    free(asf);
...
static int is_drm(char* buf, int buf_len)
{
  ...
  buf[url_len - 1] = '\0';
  asfheader_printf("DEMUX_ASFHDR_DRMLicenseURL %s\n", buf);
  return 1;
} 
...
unsigned char * STREAMHEADERBUFFER = NULL;
unsigned int STREAMHEADERBUFFERSIZE = 0;
...
int read_asf_header(demuxer_t *demuxer,struct asf_priv* asf){
  ...
  if(hdr_len < 0) {
    asfheader_printf("Header size is too small.\n");
    return 0;
  }
    
  if (hdr_len > 1024 * 1024) {
    asfheader_printf("DEMUX_ASFHDR_HeaderSizeOver1MB %d\n",
			hdr_len);
  ...
  hdr = malloc(hdr_len);
  if (!hdr) {
    asfheader_printf("DEMUX_ASFHDR_HeaderMallocFailed %d\n",
            hdr_len);
    return 0;
  }
  ...
  if (stream_eof(demuxer->stream)) {
    asfheader_printf("DEMUX_ASFHDR_EOFWhileReadingHeader\n");
    goto err_out;
  }

  if (is_drm(hdr, hdr_len))
    asfheader_printf("DEMUX_ASFHDR_DRMProtected\n");
  ...
  if ((pos = find_asf_guid(hdr, asf_ext_stream_audio, 0, hdr_len)) >= 0)
  {
    ...
    if (sh_pos > 0) {
      sh_audio_t *sh_audio;
       ...
       asfheader_printf("read_asf_header found dvr-ms audio stream header pos=%d\n", sh_pos);
  ...
  {
                {
                    ...
                    for(i=0; i<16;i++)
                        ObjectID[i]=hdr[Position+i]&0xFF;

                    for(i=0; i<8;i++) {
                        printf("ObjectSize = %u += %u\n", ObjectSize, (hdr[Position+16+i]&0xFF)<<i);
                        ObjectSize+=(hdr[Position+16+i]&0xFF)<<i;
                    }

                    printf("ObjectSize = %u\n", ObjectSize);

                    STREAMHEADERBUFFERSIZE = ObjectSize;
                    STREAMHEADERBUFFER = malloc(STREAMHEADERBUFFERSIZE);
                    for(j=0; j< STREAMHEADERBUFFERSIZE; j++)
                        STREAMHEADERBUFFER[j] = hdr[Position + j]&0xFF;

                    printf ("%s: (%d), Data = ", __FUNCTION__, ObjectSize);
                    for(j=0; j< ObjectSize;) {
                    	for (i=0; i<16 && j < ObjectSize; i++, j++)
                        	printf ("%02x ", STREAMHEADERBUFFER[j]);
                    	printf ("\n");
                    }
                }
...
ExamineStreamPropertiesObject: ObjectID                        0x11cfa9b7b7dc0791
ExamineStreamPropertiesObject: ObjectSize                      0x72
ExamineStreamPropertiesObject: StreamType                      0x11cf5b4df8699e40
ExamineStreamPropertiesObject: ErrorCorrectionType             0x11cf618fbfc3cd50
ExamineStreamPropertiesObject: TimeOffset                      0x0
ExamineStreamPropertiesObject: TypeSpecificDataLength          0x0000001c
ExamineStreamPropertiesObject: ErrorCorrectionDataLength       0x00000008
ExamineStreamPropertiesObject: Flags                           0x00000001
ExamineStreamPropertiesObject: Reserved                        0x00a2ec40
*/

    asfheader_printf("stream type: %s\n",
            asf_chunk_type(streamh->type));
    asfheader_printf("stream concealment: %s\n",
            asf_chunk_type(streamh->concealment));
    asfheader_printf("type: %d bytes,  stream: %d bytes  ID: %d\n",
            (int)streamh->type_size, (int)streamh->stream_size,
            (int)streamh->stream_no);
    asfheader_printf("unk1: %lX  unk2: %X\n",
            (unsigned long)streamh->unk1, (unsigned int)streamh->unk2);
    asfheader_printf("FILEPOS=0x%X\n", pos + start);
    // type-specific data:
    buffer = &hdr[pos];
...
//        sh_video->bih=malloc(chunksize); memset(sh_video->bih,0,chunksize);
  ...
  if (pos >= 0) {
      ...
      asfheader_printf("ASF: packets: %d  flags: %d  "
  ...
  if (pos >= 0) {
        ...
        char *ptr = &hdr[pos];
        asfheader_printf("============ ASF Stream group == START ===\n");
        ...
        asfheader_printf(" stream count=[0x%x][%u]\n", stream_count, stream_count );
        for( i=0 ; i<stream_count ; i++ ) {
          ...
          ptr += sizeof(uint32_t);
          asfheader_printf("   stream id=[0x%x][%u]\n", stream_id, stream_id );
          asfheader_printf("   max bitrate=[0x%x][%u]\n", max_bitrate, max_bitrate );
          streams[2*i] = stream_id;
          streams[2*i+1] = max_bitrate;
        }
        asfheader_printf("============ ASF Stream group == END ===\n");
  }
  free(hdr);
  hdr = NULL;
  ...
  if (memcmp(guid_buffer, asf_data_chunk_guid, 16) != 0) {
    asfheader_printf("DEMUX_ASFHDR_NoDataChunkAfterHeader\n");
    free(streams);
    streams = NULL;
    return 0;
  }
  ...
  asfheader_printf("Found movie at 0x%X - 0x%X\n",
    ...
    asfheader_printf("ASF: %d audio and %d video streams found\n", audio_streams, video_streams);
    if(!audio_streams) demuxer->audio->id=-2;  // nosound
    else if(best_audio > 0 && demuxer->audio->id == -1) demuxer->audio->id=best_audio;

    if(!video_streams) {
        if(!audio_streams) {
	        asfheader_printf("DEMUX_ASFHDR_AudioVideoHeaderNotFound\n");
	        return 0; 
        }
...
len_err_out:
    asfheader_printf("DEMUX_ASFHDR_InvalidLengthInASFHeader\n");
err_out:
    if (hdr) free(hdr);
