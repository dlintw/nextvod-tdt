...
int writeToSlave(int cpuf, int fd, unsigned int DestinationAddress, unsigned int SourceAddress, unsigned int Size)
{
	...
	lseek(cpuf, DestinationAddress, SEEK_SET);
...
int sectionToSlave(int cpuf, int fd, int * EntryPoint)
{
	...
	for(i = 0; i < IndexCounter; i++) {
		if(IndexTable[i].Size > 0 && (IndexTable[i].Flags & SEC_LOAD == SEC_LOAD)) {
			...
			writeToSlave(cpuf, fd, IndexTable[i].DestinationAddress, IndexTable[i].SourceAddress, IndexTable[i].Size);
	...
	if(BootSection != -2) {
		...
		writeToSlave(cpuf, fd, DestinationAddress, IndexTable[BootSection].SourceAddress, IndexTable[BootSection].Size);
