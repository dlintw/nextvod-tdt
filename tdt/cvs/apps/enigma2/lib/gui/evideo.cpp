...
#else
/* Dagobert: Setting the vmpeg procs leads everytime to a SetOutputWindow
 * in the player. This is not necessary and maybe it confuses the player
 * when the values does not match each other.
 */
   char filename[128];
   snprintf(filename, 128, "/proc/stb/vmpeg/%d/dst_all", m_decoder);
   FILE *f = fopen(filename, "w");
   if (!f)
   {
	   eDebug("failed to open %s - %m", filename);
