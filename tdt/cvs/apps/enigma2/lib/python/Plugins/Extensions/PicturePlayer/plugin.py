from enigma import eTimer, loadPic, getExif, fbClass
...
class ThumbView(Screen):
	skin = """
		<screen position="0,0" size="720,576" flags="wfNoBorder" title="ThumbView" >
			...
			<widget name="frame" position="0,0" size="700,576" pixmap="pic_frame.png" zPosition="1" alphatest="on" />
...
class PicView(Screen):
	skin = """
		<screen position="0,0" size="720,576" flags="wfNoBorder" title="PicturePlayer" >
			...
			<widget name="point" position="40,515" size="14,14" zPosition="1" pixmap="skin_default/icons/record.png" alphatest="on" />
			<widget name="file" position="100,510" size="400,30" font="Regular;20" halign="center" zPosition="2" transparent="1" />
			<ePixmap position="500,515" size="36,20" pixmap="skin_default/buttons/key_info.png" zPosition="2" alphatest="on" />
			<ePixmap position="550,515" size="20,20" pixmap="skin_default/icons/ico_mp_rewind.png"  zPosition="2" alphatest="on" />
			<widget name="play" position="575,515" size="20,20" pixmap="skin_default/icons/ico_mp_play.png"  zPosition="2" alphatest="on" />
			<widget name="pause" position="600,515" size="20,20" pixmap="skin_default/icons/ico_mp_pause.png"  zPosition="1" alphatest="on" />
			<ePixmap position="625,515" size="20,20" pixmap="skin_default/icons/ico_mp_forward.png"  zPosition="2" alphatest="on" />
		</screen>"""
	...
	def __init__(self, session, filelist, name, path):
		...
		thread.start_new_thread(self.decodePic, ())
	...
	def decodePic(self):
		fbClass.getInstance().blitFullscreenJpg(self.list[self.index][1]);
...
class picmain(Screen):
	...
	def __init__(self, session):
		...
		self.filelist = FileList(currDir, matchingPattern = "(?i)^.*\.(jpeg|jpg|jpe)")
...
def filescan(**kwargs):
	...
	return \
		LocalScanner(mimetypes = ["image/jpeg"],
			paths_to_scan = 
				[
...
def Plugins(**kwargs):
	return \
		[PluginDescriptor(name="PicturePlayer", description="Picture Viewer (JPG)", icon="pictureplayer.png", where = PluginDescriptor.WHERE_PLUGINMENU, fnc=main),
