...
if ENABLE_VDR1711
	...
	if [ ! -e $(buildprefix)/Archive/vdr-1.7.14_mod.bz2 ]; then \
		cd $(buildprefix)/Archive/; \ 
		wget http://kathif.vs120005.hl-users.com/test/vdr-1.7.14_mod.bz2 ; \
	fi; 
	cd $(DIR_vdr) && \
	tar jxf $(buildprefix)/Archive/vdr-1.7.14_mod.bz2
	cd $(DIR_vdr)/vdr-1.7.14 && \
		$(MAKE_OPTS) $(MAKE) vdr plugins prefix=$(targetprefix)
	touch $@
endif
...
if ENABLE_VDR1711
	...
	$(MAKE) -C $(DIR_vdr)/vdr-1.7.14 all install install-plugins prefix=$(targetprefix) DESTDIR=$(targetprefix)
	cp $(appsdir)/vdr/vdr-1.7.14/PLUGINS/src/remote/misc/remote.conf $(targetprefix)/var/vdr
	mkdir -p $(targetprefix)/usr/lib/vdr
	mkdir -p $(targetprefix)/usr/local/bin
	cp $(appsdir)/vdr/vdr-1.7.14/PLUGINS/lib/libvdr*1.7.14 $(targetprefix)/usr/lib/vdr
#	cp $(appsdir)/vdr/vdr-1.7.14/sources.conf $(targetprefix)/var/vdr
#	cp $(appsdir)/vdr/vdr-1.7.14/channels.conf $(targetprefix)/var/vdr
#	cp $(appsdir)/vdr/vdr-1.7.14/keymacros.conf $(targetprefix)/var/vdr
endif
	cp -rd $(appsdir)/vdr/fonts $(targetprefix)/usr/share
	cp -rd $(appsdir)/vdr/vdr-1.7.14/vdr $(targetprefix)/usr/local/bin
...
#	cp -rd $(targetprefix)/usr/lib/vdr/libvdr-vfd.so.1.7.14 $(prefix)/release_vdr/usr/lib/vdr/
...
#	cp $(buildprefix)/root/release/rcS_vdr_ufs910 $(prefix)/release_vdr/etc/init.d/rcS
	cp $(buildprefix)/root/release/rcS_vdr_1.7.14_ufs910 $(prefix)/release_vdr/etc/init.d/rcS
...
if ENABLE_VDR1711
	...
	$(MAKE) -C $(DIR_vdr)/vdr-1.7.14 clean clean-plugins
endif
	-rm .deps/vdr*
